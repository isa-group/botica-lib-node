...
export class Bot {
  ...
  /**
   * Registers the given listener for the provided order. If no order argument is specified, the
   * order is taken from the main configuration file.
   *
   * @param orderListener the listener to register
   * @param order the order to listen to (optional)
  ...
  /**
   * Publishes an order with the given message. If no key or order argument is specified, they are
   * taken from the main configuration file.
   *
   * @param message the message of the order
   * @param key the key to publish the order with (optional)
   * @param order the order to publish (optional)
   */
  async publishOrder(
    message: string,
    key?: string,
